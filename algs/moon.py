...
class Moon(FedAvg):
    def __init__(self, dataset_train, dataset_test, dict_users_train, dict_users_test, model, args, idxs_all_users=None):
        ...
        self.moon_alpha = args.moon_alpha
    ...
    def choose_users(self):
        func_info = "{}->{}".format(Moon.__qualname__, sys._getframe().f_code.co_name)
    ...
    def loss_func(self, log_probs, labels, z_now=None, z_glob=None, z_prev=None):
        cos = torch.nn.CosineSimilarity(dim=-1)
        loss_ce = self.loss_ce(log_probs, labels)

        if z_now is None:
            loss = loss_ce
            return loss, {'loss': loss.item(),'loss_ce':loss_ce.item(), 'loss_con': loss_ce.item()}
        else:
            sims0 = cos(z_now, z_prev)
            sims1 = cos(z_now, z_glob)
            
            sims = 2.0 * torch.stack([sims0, sims1], dim=1)
            sim_labels = torch.LongTensor([1] * z_now.shape[0])
            sim_labels = sim_labels.to(z_now.device)

            loss_con = self.loss_ce(sims, sim_labels)
            
            loss = loss_ce + self.moon_alpha * loss_con
            return loss, {'loss': loss.item(),'loss_ce':loss_ce.item(), 'loss_con': loss_con.item()}
    ...
    def local_update(self, model, epoch, idx_data, idx_test_data, user_id):
        ...
        for _ in range(1 + local_eps * (epoch - 1), local_eps + 1 + local_eps * (epoch - 1)):
            ...
            for batch_idx, (images, labels) in enumerate(loader_local_train):
                ...
                model.zero_grad()
                log_probs, z_now = model.forward_with_feat(images)
                
                if len(self.last_model_list) != self.num_users:
                    loss, loss_result = self.loss_func(log_probs, labels)
                else:
                    # model_prev = copy.deepcopy(self.model)
                    # model_prev = load_model_from_params(model_prev, self.params_array[user_id])
                    with torch.no_grad():
                        _, z_glob = self.model.forward_with_feat(images)
                        _, z_prev = self.last_model_list[user_id].forward_with_feat(images)
                    loss, loss_result = self.loss_func(log_probs, labels, z_now, z_glob, z_prev)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.args.epochs + 1):
            ...
            for idx in idxs_users:
                ...
                self.last_model_list[idx] = model_local
            model_list.append(copy.deepcopy(dict(self.model.named_parameters())))
            self.params_array = get_models_array(model_list)
            if self.args.save_clients or epoch <= self.start_epoch + 5:
                np.save(os.path.join(self.base_dir, "models_{}.npy".format(epoch)), self.params_array)
