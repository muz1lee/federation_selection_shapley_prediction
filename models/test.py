...
def test_center(model, data_loader, return_probs=False, global_test=False):
    model.eval()
    # testing
    test_loss = 0
    correct = 0
    ...
    with torch.no_grad():
        for idx, (images, labels) in enumerate(data_loader):
            ...
            # sum up batch loss
            loss = F.cross_entropy(log_probs, labels, reduction='mean')
    ...
    if global_test:
        results['global_test_loss'] = test_loss
        results['global_test_acc'] = accuracy
    else:
        results['test_loss'] = test_loss
        results['test_acc'] = accuracy

    if return_probs:
        return results, torch.cat(probs)
    return results
...
def test_img_local(net_g, dataset, args, user_idx=-1, idxs=None):
    ...
    test_loss = 0
    correct = 0
    data_loader = DataLoader(DatasetSplit(dataset, idxs), batch_size=args.bs_test, shuffle=False)
...
def test_img_ensemble_all(net_local_list, args, dataset_test):
    probs_all = []  # probability
    preds_all = []  # predict class
    for idx in range(args.num_users):
        net_local = net_local_list[idx]
        net_local.eval()
